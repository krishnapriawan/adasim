...
public class PriorityRoutingAlgorithm extends AbstractRoutingAlgorithm {

	private final static Logger logger = Logger.getLogger(PriorityRoutingAlgorithm.class);
	 ...
	 * the path.
	 */
	public PriorityRoutingAlgorithm() {
		this(2);
	}
	 ...
	 * @param lookahead
	 */
	public PriorityRoutingAlgorithm( int lookahead ){
	 ...
	 * @param lookahead
	 */
	public PriorityRoutingAlgorithm( int lookahead, int recomp ){
		this.lookahead = lookahead;
		this.recompute = recomp;
		this.steps = 0;
	}
	
	public PriorityRoutingAlgorithm(boolean withDelay) {
		this(0, withDelay);
	}
	
	public PriorityRoutingAlgorithm(int lookahead, boolean withDelay) {
		this(lookahead, lookahead);
...
//					logger.info(" NEWW - OPEN : Road :"+ node.getID());
				...
				int t = dist[current] + ( withDelay ? 
							(depth <= l ? 
								node.getCurrentDelay(Vehicle.class) : 
								node.getDelay(Vehicle.class)) : 
							0);
